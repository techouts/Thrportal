...
export function AssignmentProjectSection() {
  ...
  const [selectedAllocationForEdit, setSelectedAllocationForEdit] =
    useState<ProjectAllocation | null>(null);
  ...
  const { data: clientsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      // return data || [];
      const response = await NodeApiClient.get("/crm/clients/list/id-name");
  ...
  const { data: accountsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await NodeApiClient.get(
        `/crm/clients/${selectedClient}/accounts`
      );
  ...
  const { data: projectsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      // return data || [];
      const response = await NodeApiClient.get(
        `/crm/clients/accounts/${selectedAccount}/projects`,
        {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // if (error) throw error;
      const response = await NodeApiClient.get(
        `/allocations/project/${selectedProject}`
      );

      const data = response.data ?? [];

      const mappedAllocations: ProjectAllocation[] = (data || []).map(
        (alloc: any) => ({
          id: alloc.id,
          employeeName:
            alloc.profiles?.display_name ||
            `${alloc.profiles?.first_name || ""} ${
              alloc.profiles?.last_name || ""
            }`.trim() ||
            "Unknown",
  ...
  const scorecardData = useMemo(() => {
    ...
    const shadowResources = allocations.filter(
      (a) => a.type === "SHADOW"
    ).length;
    const underAllocated = allocations.filter(
      (a) => a.allocationPct < 50
    ).length;
    const overAllocated = allocations.filter(
      (a) => a.allocationPct > 100
    ).length;
    ...
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-muted-foreground text-center">
              Select Client, Account, and Project, then click Search to view
              allocations
            </p>
          </CardContent>
        </Card>
      ...
      {hasSearched && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
                ...
                {allocations.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {allocations.map((allocation) => (
                      <div
                        key={allocation.id}
                        className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 p-4 border rounded-lg"
                      >
                        ...
                        <div className="w-full md:w-28">
                          ...
                          <Progress
                            value={Math.min(allocation.allocationPct, 100)}
                            className="w-full"
                          />
          ...
          <ScorecardMobileWrapper type="project" data={scorecardData} />
        </div>
      )}
