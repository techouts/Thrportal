...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchProfile = async (
    userId: string
  ): Promise<ApiProfile & { role?: string; role_title?: string }> => {
  ...
  const profileToUser = async (
    userId?: string
    // profile: Profile & { password_change_required?: boolean },
  ...
  useEffect(() => {
    const bootstrapAuth = async () => {
      ...
      try {
        ...
        const userData = await profileToUser(userId);
        setUser(userData);
      } catch (err) {
  ...
  const signIn = async (email: string, password: string): Promise<User> => {
    ...
    try {
      ...
      const response = await NodeApiClient.post("/auth/login", {
        email,
        password,
      });
      ...
      const userData = await profileToUser(apiUser.id);
  ...
  const signUp = async (
    ...
    if (isDevAuthMode()) {
      throw new Error(
        "Sign up not available in dev mode. Use existing dev accounts."
      );
    }
  ...
  const signOut = async (): Promise<void> => {
    ...
    // }
    try {
      await NodeApiClient.post("/auth/logout");
    } catch (error) {
      console.warn("[AUTH] Logout API failed, forcing local logout");
    } finally {
