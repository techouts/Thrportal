...
export const allocationService = {
  ...
  async searchResources(searchTerm: string): Promise<ResourceOption[]> {
    ...
    const response = await NodeApiClient.get("/auth/users/all", {
      params: {
        search: searchTerm,
      },
    });
  ...
  async getEmployeeById(employeeId: string): Promise<EmployeeDetails | null> {
    ...
    //   .from("profiles")
    //   .select(
    //     "id, display_name, first_name, last_name, role_title, department, employee_code, email"
    //   )
    //   .eq("id", employeeId)
    //   .single();
    ...
    //   return null;
    // }
    const { data } = await NodeApiClient.get(`/profile/${employeeId}/detail`);
  ...
  async getEmployeeAllocations(
    ...
    const { data } = await NodeApiClient.get(
      `/employeeAllocations/employee/${employeeId}`,
      {
        params: {
          endDate: today, // e.g. "2026-01-31"
        },
      }
    );
    ...
    return (data || []).map((allocation) => ({
      ...
      clientName:
        (allocation.crm_projects as any)?.crm_clients?.name || "Unknown Client",
  ...
  async getRoles(): Promise<RoleOption[]> {
    ...
    // return data || [];
    try {
      const response = await NodeApiClient.get("/roles");
   ...
   */
  async getActiveAllocationsForResource(
    resourceId: string
    ...
    // }
    const { data } = await NodeApiClient.get(
      `/employeeAllocations/employee/${resourceId}`,
      {
        params: {
          endDate: today, // e.g. "2026-01-31"
        },
  ...
  async createAllocation(input: CreateAllocationInput) {
    ...
    // return data;
    const response = await NodeApiClient.post("/allocations", {
      project_id: input.projectId,
      employee_id: input.resourceId,
      role_id: input.roleId,
      allocation_pct: input.allocationPct,
  ...
  async searchProjects(searchTerm: string): Promise<ProjectOption[]> {
    ...
    //   return [];
    // }
    const { data } = await NodeApiClient.get("/employeeAllocations/projects", {
      params: {
        select: searchTerm,
      },
    });
  ...
  async checkExistingAllocation(
    ...
    // }
    const { data } = await NodeApiClient.get(
      "/allocations/allocation-ids/filtered",
      {
        params: {
          employee_id: employeeId,
          project_id: projectId,
          end_date: today, // backend checks >= or null
        },
      }
    );
