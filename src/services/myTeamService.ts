...
export class MyTeamService {
  ...
  async getTimesheetEntries(timesheetId: string): Promise<ApiResponse<any>> {
    try {
      ...
      const { data } = await NodeApiClient.get(
        `/timesheets-v2/${timesheetId}/entries`
      );
  ...
  async getApprovalQueues(
    ...
    try {
      ...
      // }
      const { data: teamMembers } = await NodeApiClient.get(
      ...
      if (teamMemberIds.length > 0) {
        ...
        let timesheets: any[] = [];
        try {
          // Map status filter exactly like Supabase logic
          ...
          if (statusFilter === "rejected") {
            ...
            // "all"
            statusParam = "SUBMITTED,REJECTED";
          }

          const { data } = await NodeApiClient.get("/timesheets-v2", {
            params: {
              employee_id: teamMemberIds.join(","), // IN (...)
              status: statusParam,
            },
          });
        ...
        if (timesheets && timesheets.length > 0) {
          const employeeIds = [
            ...new Set(timesheets.map((ts) => ts.employee_id)),
          ];
          ...
          // }
          const { data: profiles } = await NodeApiClient.get("/auth/by-ids", {
            params: {
              ids: employeeIds.join(","), // same as Supabase .in("id", ...)
            },
          });
          ...
          timesheetApprovals = timesheets.map((ts) => {
            ...
            return {
              ...
              description: `${ts.total_hours || 0} hours total (${
                ts.billable_hours || 0
              } billable)${
                ts.submission_comment ? ` - ${ts.submission_comment}` : ""
              }`,
  ...
  async approveRequest(
    ...
    try {
      ...
      if (type === "timesheet") {
        ...
        // }
        const { data: timesheet } = await NodeApiClient.get(
          `/timesheets-v2/meta/${requestId}`
        );
  ...
  async bulkApprove(
    ...
    try {
      if (type === "timesheet") {
        ...
        // }
        const { data: timesheets } = await NodeApiClient.get(
          `/timesheets-v2/meta/${requestIds.join(",")}`
        );
